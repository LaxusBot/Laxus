...
object Database: AutoCloseable {
    ...
    fun start(resource: String = "database.conf") {
        ...
        if(config.nullBoolean("shutdown.hook") == true) {
            createShutdownHook()
        }

        log.debug("Logging in using configuration: $resource")

        val url = config.string("url.prefix") + config.string("url.path")
        val user = config.string("login.user")
        val pass = config.string("login.pass")

        log.info("Connecting to database: $url")
        _connection = DriverManager.getConnection(url, user, pass)

        config.list("tables").asSequence().mapNotNull { it.string }.map { path ->
            return@map requireNotNull(loadClass(path)) {
