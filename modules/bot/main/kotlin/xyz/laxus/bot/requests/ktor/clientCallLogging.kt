...
class ClientCallLogging private constructor(
    ...
    class Configuration internal constructor() {
        ...
        @ClientCallLoggingDsl
        fun name(klass: KClass<*>) = name(requireNotNull(klass.qualifiedName) {
            "Cannot resolve qualified name for class: $klass"
        })

        @ClientCallLoggingDsl
        inline fun send(block: ClientCallLogging.PipelineConfiguration.() -> Unit) = send.block()

        @ClientCallLoggingDsl
        inline fun receive(block: ClientCallLogging.PipelineConfiguration.() -> Unit) = receive.block()

        @ClientCallLoggingDsl
        inline fun error(block: ClientCallLogging.PipelineConfiguration.() -> Unit) = error.block()
    ...
    companion object Feature: HttpClientFeature<Configuration, ClientCallLogging> {
        ...
        override fun prepare(block: Configuration.() -> Unit) = Configuration().also(block).build()
...
@ClientCallLoggingDsl
inline fun <T: HttpClientEngineConfig> HttpClientConfig<T>.logging(
