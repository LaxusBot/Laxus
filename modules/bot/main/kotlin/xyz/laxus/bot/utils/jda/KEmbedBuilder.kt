...
class KEmbedBuilder: Appendable {
    ...
    val length: Int get() {
        ...
        synchronized(fields) {
            length = fields.asSequence()
                .map { f -> f.name.length + f.value.length }
                .fold(length) { acc, i -> acc + i }
        }
    ...
    @MessageDsl var title: String? = null
    @MessageDsl var url: String? = null
    @MessageDsl var thumbnail: String? = null
    @MessageDsl var image: String? = null
    @MessageDsl var author: Entity? = null
    @MessageDsl var footer: Entity? = null
    @MessageDsl var time: TemporalAccessor? = null
    @MessageDsl var color: Color? = null

    @PublishedApi internal fun build(): MessageEmbed = with(EmbedBuilder()) {
        val(title, url, time, color, author, thumbnail, footer, image) = this@KEmbedBuilder
    ...
    private operator fun component1() = title
    private operator fun component2() = url
    private operator fun component3() = time
    private operator fun component4() = color
    private operator fun component5() = author
    private operator fun component6() = thumbnail
    private operator fun component7() = footer
    private operator fun component8() = image
    ...
    data class Entity @PublishedApi internal constructor(
        ...
        inline fun value(lazy: () -> String): Entity {
            val value = lazy()
            this.value = if(value.isBlank()) ZERO_WIDTH_SPACE else value
            return this
        }
