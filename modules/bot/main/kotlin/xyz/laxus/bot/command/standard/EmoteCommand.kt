...
import xyz.laxus.bot.command.MustHaveArguments
...
import xyz.laxus.utils.name
import xyz.laxus.utils.toChars
import xyz.laxus.utils.toHexString
...
class EmoteCommand: Command(StandardGroup) {
    ...
    override suspend fun execute(ctx: CommandContext) {
        ...
        if(args matches emoteRegex) {
            ...
            commandErrorIf(emotes.size < 1) { "The specified emote was fake, or could not be retrieved!" }
            val emote = emotes[0]
            ctx.reply(embed {
            ...
            commandErrorIf(unicode.length > 10) { "Cannot process more than 10 characters!" }
