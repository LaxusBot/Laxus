...
class ImageCommand(client: HttpClient): Command(StandardGroup) {
    ...
    private val api = GoogleImageRequester(client)
    ...
    override suspend fun execute(ctx: CommandContext) {
        ...
        commandErrorIf(results.isEmpty()) { "No results were found for '**$query**'!" }
        ctx.reply(message {
            append("${Laxus.Success} ${ctx.author.asMention}")
            embed {
                if(ctx.isGuild) color { ctx.member.color }
