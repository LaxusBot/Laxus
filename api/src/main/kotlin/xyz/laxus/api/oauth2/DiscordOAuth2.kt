...
class DiscordOAuth2(private val client: HttpClient) {
    private companion object {
        ...
        private val contentType = Application.Json.withCharset(Charsets.UTF_8)
    }
    ...
    suspend fun guild(id: Long, session: OAuth2Session): Guild? {
        ...
        val json = response.use {
            ...
            return@use response.readBytes().readJSArray(response.charset() ?: Charsets.UTF_8)
        }

        if(json.any { id == (it as? JSObject)?.long("id") }) {
            return Laxus.JDA.getGuildById(id)
        }
